...
class Year4x2WidgetProvider : HomeWidgetProvider() {
  ...
  override fun onUpdate(
    ...
    appWidgetIds.forEach { widgetId ->
      ...
      val dateText     = widgetData.getString(KEY_DATE_TEXT, formatDate()) ?: formatDate()
      val daysLeftText = widgetData.getString(KEY_DAYS_LEFT_TEXT, "${daysLeftInYear()} days left") ?: ""
  ...
  private fun drawWidgetBitmap(
    ...
    val radius = (minOf(cellW, cellH) * fillRatio / 2f)
        .coerceAtLeast(2f)
        .coerceAtMost(minOf(cellW, cellH) / 2f)
    ...
    for (index in 0 until columns * rows) {
      ...
      val cx  = gridLeft + cellW * col + cellW / 2f
