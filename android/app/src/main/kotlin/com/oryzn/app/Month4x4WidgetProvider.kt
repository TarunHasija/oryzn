...
// Dot grid columns
private const val DOT_COLUMNS = 9
private const val MIN_DOT_COLUMNS = 6

// Dot diameter in dp  ← change this to make dots bigger / smaller
private const val DOT_DIAMETER_DP = 16f

// Gap between dots in dp  ← change this to adjust spacing between dots
private const val DOT_GAP_DP = 7f
...
class Month4x4WidgetProvider : HomeWidgetProvider() {
  ...
  override fun onUpdate(
    ...
    appWidgetIds.forEach { widgetId ->
      val views = RemoteViews(context.packageName, R.layout.month_4x4_widget)
      ...
      val fallbackDaysLeftText = "${daysLeftInMonth()} days left"
      val dateText = widgetData.getString(KEY_DATE_TEXT, fallbackDateText) ?: fallbackDateText
      val daysLeftText = widgetData.getString(KEY_MONTH_DAYS_LEFT_TEXT, fallbackDaysLeftText) ?: fallbackDaysLeftText
  ...
  override fun onReceive(context: Context, intent: android.content.Intent) {
    ...
    if (intent.action == android.content.Intent.ACTION_CONFIGURATION_CHANGED) {
      ...
      val ids = manager.getAppWidgetIds(android.content.ComponentName(context, Month4x4WidgetProvider::class.java))
  ...
  private fun drawWidgetBitmap(
    ...
    val dayIndex   = Calendar.getInstance().get(Calendar.DAY_OF_MONTH) - 1  // 0-based

    // Pick column count that best fills the available grid area with square cells.
    val aspect = gridW / gridH
    val approxColumns = sqrt(daysInMonth * aspect).roundToInt()
    ...
    var rows = ceil(daysInMonth.toDouble() / columns).toInt().coerceAtLeast(1)
    var bestCell = minOf(gridW / columns, gridH / rows)
    var bestFillArea = (bestCell * columns) * (bestCell * rows)

    for (candidate in start..end) {
      val candidateRows = ceil(daysInMonth.toDouble() / candidate).toInt().coerceAtLeast(1)
  ...
  private fun daysLeftInMonth(): Int {
    ...
    return (today.getActualMaximum(Calendar.DAY_OF_MONTH) - today.get(Calendar.DAY_OF_MONTH)).coerceAtLeast(0)
  }
  ...
  companion object {
    ...
    private const val KEY_MONTH_DAYS_LEFT_TEXT = "widget_month_days_left_text"
  }
}
